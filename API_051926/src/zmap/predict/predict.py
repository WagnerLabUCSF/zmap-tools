...
def _evaluate_label_predictions_common(
    adata_query: ad.AnnData,
    *,
    label_space: str,
    ...
    def _base_col(lbl: str) -> str:
        if label_suffix is not None and str(label_suffix) != "":
            return f"{lbl}_{label_suffix}"
        return lbl

    labels_base = _base_col(label_space)
    ...
    if eval_group_col:
        if eval_group_col not in adata_query.obs.columns:
            raise ValueError(f"eval_group_col={eval_group_col!r} not found in adata_query.obs.")
        eval_df["group"] = adata_query.obs.loc[base_mask, eval_group_col].astype(str).values

    if eval_label_map:
        eval_df["truth"] = eval_df["truth"].map(lambda x: eval_label_map.get(str(x), str(x)))
        eval_df["pred"] = eval_df["pred"].map(lambda x: eval_label_map.get(str(x), str(x)))
    ...
    if eval_restrict_to_truth_labels:
        allowed_initial = set(eval_df["truth"].astype(str).unique().tolist())
        keep_mask = eval_df["pred"].astype(str).isin(allowed_initial)
    ...
    if eval_min_cells_per_label is not None and eval_min_cells_per_label > 0:
        ...
        rare_eval_labels = eval_counts[eval_counts < int(eval_min_cells_per_label)].index.astype(str)
    ...
    class_indices = {str(cls): i for i, cls in enumerate(class_order)}
    ...
    metrics_dict = {
        "Aggregate Metrics": pd.DataFrame({
            ...
            "Score": [
                accuracy_score(true_labels_values, predicted_labels_values),
                precision_score(true_labels_values, predicted_labels_values, average="macro", zero_division=0),
                recall_score(true_labels_values, predicted_labels_values, average="macro", zero_division=0),
                f1_score(true_labels_values, predicted_labels_values, average="macro", zero_division=0),
                macro_auroc,
            ],
        }),
    ...
    if metrics_dict["Run Summary"] is not None:
        ...
        metrics_dict["Run Summary"]["Params"]["eval_restrict_to_truth_labels"] = eval_restrict_to_truth_labels
        metrics_dict["Run Summary"]["Params"]["eval_truth_min_cells_per_label"] = eval_truth_min_cells_per_label
        ...
        metrics_dict["Run Summary"]["Coverage"]["eval_excluded_rare_prediction_n"] = eval_excluded_rare_n
        metrics_dict["Run Summary"]["Coverage"]["eval_excluded_rare_group_n"] = eval_excluded_rare_group_n
...
def predict_labels_kNN(
    ...
    *,
    ...
    tissue_mode: str = "hard",     # "hard" | "soft"
    tissue_penalty_lambda: float = 1.0,
    tissue_prediction_col: str | None = None,
    omit_labels: list[str] | None = ['unknown', 'nan', 'unassigned', 'blastomere', 'blastomeres'],
    ...
    - Set tissue_aware=True to restrict/bias non-tissue label transfer by tissue.
      You must pass tissue_col for an existing ref/query tissue column, or set
      predict_tissue_level=True to infer query tissue labels first. Do not use
      tissue_aware=True when ref_label_col itself is the tissue column.
    """
    ...
    def _values_digest(values) -> list[int]:
        s = pd.Series(values, dtype="string").fillna("<NA>")
        if len(s) == 0:
            return [0, 0]
        h = pd.util.hash_pandas_object(s, index=False).to_numpy(dtype=np.uint64)
        return [int(len(s)), int(np.bitwise_xor.reduce(h))]

    def _vote_labels_from_neighbors(ref_values: np.ndarray, idx: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        uniq = np.sort(pd.Series(ref_values).dropna().astype(str).unique())
        if len(uniq) == 0:
            raise ValueError("No labels available for KNN voting.")
        pred = np.empty(idx.shape[0], dtype=object)
        max_prob = np.zeros(idx.shape[0], dtype=float)
        for i, nbrs in enumerate(idx):
            vals = np.asarray(ref_values[nbrs], dtype=str)
            ids = np.searchsorted(uniq, vals)
            counts = np.bincount(ids, minlength=len(uniq)).astype(float)
            total = counts.sum()
            if total <= 0:
                pred[i] = pd.NA
                continue
    ...
    def _global_knn(X_ref_use, X_query_use, k_use: int):
        return knn_search(
            X_ref_use,
            X_query_use,
    ...
    def _hard_tissue_knn(X_ref_use, X_query_use, ref_tissue_use, query_tissue_use):
        idx_out = np.full((X_query_use.shape[0], n_neighbors), -1, dtype=np.int64)
        dist_out = np.full((X_query_use.shape[0], n_neighbors), np.nan, dtype=np.float32)
        meta = None
        global_idx = None
        global_dist = None
        for tissue in np.unique(query_tissue_use.astype(str)):
            q_rows = np.flatnonzero(query_tissue_use.astype(str) == str(tissue))
            if q_rows.size == 0:
                continue
            r_rows = np.flatnonzero(ref_tissue_use.astype(str) == str(tissue))
            if r_rows.size >= int(n_neighbors):
                idx_local, dist_local, meta_local = _global_knn(
                    X_ref_use[r_rows],
                    X_query_use[q_rows],
                    int(n_neighbors),
                )
                ...
                meta = meta or meta_local
            else:
                if global_idx is None or global_dist is None:
                    global_idx, global_dist, meta = _global_knn(
                        X_ref_use,
                        X_query_use,
                        int(n_neighbors),
                    )
        ...
        return idx_out, dist_out, meta

    def _soft_tissue_knn(X_ref_use, X_query_use, ref_tissue_use, query_tissue_use):
        global_k = min(max(int(n_neighbors) * 3, int(n_neighbors) + 5), X_ref_use.shape[0])
        idx_global, dist_global, meta = _global_knn(X_ref_use, X_query_use, global_k)
        idx_out = np.full((X_query_use.shape[0], n_neighbors), -1, dtype=np.int64)
        dist_out = np.full((X_query_use.shape[0], n_neighbors), np.nan, dtype=np.float32)
        ref_tissue_str = ref_tissue_use.astype(str)
        query_tissue_str = query_tissue_use.astype(str)
        for i in range(X_query_use.shape[0]):
            row_idx = idx_global[i]
            row_dist = np.asarray(dist_global[i], dtype=np.float32)
            ok = row_idx >= 0
            if not np.any(ok):
                continue
            cand_idx = row_idx[ok]
            cand_dist = row_dist[ok].copy()
            mismatch = ref_tissue_str[cand_idx] != query_tissue_str[i]
            cand_dist[mismatch] += float(tissue_penalty_lambda)
            order = np.argsort(cand_dist)[: int(n_neighbors)]
            idx_out[i, : len(order)] = cand_idx[order]
            dist_out[i, : len(order)] = cand_dist[order]
        return idx_out, dist_out, meta
    ...
    if tissue_mode not in {"hard", "soft"}:
        raise ValueError("tissue_mode must be one of {'hard', 'soft'}.")
    if tissue_aware and tissue_col is None and not bool(predict_tissue_level):
        raise ValueError(
            "tissue_aware=True requires tissue_col, or set predict_tissue_level=True "
            "to predict query tissue labels before tissue-aware KNN."
        )
    if tissue_penalty_lambda < 0:
        raise ValueError("tissue_penalty_lambda must be >= 0.")
    ...
    if omit_labels:
        omit_set = {str(x).lower() for x in omit_labels}
        ref_keep_mask = ~(
            ref_labels_full.isna()
            | ref_labels_full.astype(str).str.lower().isin(omit_set)
        )
    else:
    ...
    tissue_col_use = str(tissue_col) if tissue_col is not None else "ZMAP_Tissue"
    ref_tissue = None
    query_tissue = None
    tissue_digest = None
    if tissue_aware:
        if str(ref_label_col) == str(tissue_col_use):
            raise ValueError(
                "tissue_aware=True is not meaningful when ref_label_col is the "
                f"same as tissue_col ({tissue_col_use!r}). For tissue-level "
                "prediction, run predict_labels_kNN with tissue_aware=False."
            )
        if tissue_col_use not in adata_ref.obs:
            raise KeyError(
                f"tissue_col '{tissue_col_use}' not found in adata_ref.obs "
                "(required for tissue_aware=True)."
            )
        ref_tissue = adata_ref.obs[tissue_col_use].astype(object)[ref_keep_mask].astype(str).to_numpy()
        if predict_tissue_level:
            tissue_k = min(int(n_neighbors), int(X_ref.shape[0]))
            tissue_idx, _, tissue_meta = _global_knn(X_ref, adata_query.obsm[query_basis], tissue_k)
            query_tissue, tissue_prob = _vote_labels_from_neighbors(ref_tissue, tissue_idx)
            tissue_pred_col = (
                str(tissue_prediction_col)
                if tissue_prediction_col is not None
                else f"{tissue_col_use}_predicted"
            )
            adata_query.obs[tissue_pred_col] = query_tissue.astype(str)
            adata_query.obs[f"{tissue_pred_col}_prob"] = tissue_prob
            adata_query.obs[tissue_col_use] = query_tissue.astype(str)
            adata_query.uns.setdefault("zmap_labels", {}).setdefault(space, {})
            adata_query.uns["zmap_labels"][space]["Tissue Prediction"] = {
                "tissue_col": tissue_col_use,
                "output_col": tissue_pred_col,
                "n_neighbors": int(tissue_k),
                "knn_backend_requested": tissue_meta.get("backend_requested", knn_backend),
                "knn_device_requested": tissue_meta.get("device_requested", knn_device),
                "knn_backend_used": tissue_meta.get("backend_used", "sklearn"),
                "knn_device_used": tissue_meta.get("device_used", "cpu"),
            }
        else:
            if tissue_col_use not in adata_query.obs:
                raise KeyError(
                    f"tissue_col '{tissue_col_use}' not found in adata_query.obs. "
                    "Provide query tissue annotations or set predict_tissue_level=True."
                )
            query_tissue = adata_query.obs[tissue_col_use].astype(str).to_numpy()
        tissue_digest = {
            "tissue_col": tissue_col_use,
            "predict_tissue_level": bool(predict_tissue_level),
            "ref_tissue": _values_digest(ref_tissue),
            "query_tissue": _values_digest(query_tissue),
        }
    ...
    if 'zmap_neighbors' in adata_query.uns:
        ...
        same_config = (
            ...
            cache.get('tissue_aware', False) == bool(tissue_aware) and
            cache.get('tissue_mode') == (str(tissue_mode) if tissue_aware else None) and
            cache.get('tissue_digest') == tissue_digest
        )
        if same_config:
    ...
    if not reuse_neighbors:
        if tissue_aware:
            print(f"Computing new tissue-aware kNN graph on filtered reference ({tissue_mode}).")
            if ref_tissue is None or query_tissue is None:
                raise ValueError("Internal error: tissue-aware KNN is missing tissue annotations.")
            if tissue_mode == "hard":
                neighbor_indices, distances, knn_meta = _hard_tissue_knn(
                    X_ref,
                ...
                neighbor_indices, distances, knn_meta = _soft_tissue_knn(
                    X_ref,
                    adata_query.obsm[query_basis],
                    ref_tissue,
                    query_tissue,
            ...
            neighbor_indices, distances, knn_meta = _global_knn(
                X_ref,
                adata_query.obsm[query_basis],
                int(n_neighbors),
            )
        ...
        adata_query.uns['zmap_neighbors'] = {
            ...
            'tissue_mode': (str(tissue_mode) if tissue_aware else None),
            'tissue_digest': tissue_digest,
        }
    ...
    run_summary = {
        ...
        "Params": {
            ...
            "knn_device_used": knn_meta.get("device_used", "cpu"),
            "tissue_aware": bool(tissue_aware),
            "tissue_col": (tissue_col_use if tissue_aware else None),
            "predict_tissue_level": bool(predict_tissue_level),
            "tissue_mode": (str(tissue_mode) if tissue_aware else None),
            "tissue_penalty_lambda": (float(tissue_penalty_lambda) if tissue_aware else None),
    ...
    # ---------- evaluation ----------
    if evaluate:
        _evaluate_label_predictions_common(
            adata_query,
            label_space=space,
...
def annotate_with_zmap(
    ...
    *,
    ...
    pk.setdefault("knn_backend", "auto")
    pk.setdefault("knn_device", "auto")
