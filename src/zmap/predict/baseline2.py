...
def _time_probs_to_stage_probs(
    *,
    p_time: np.ndarray,
    time_classes: np.ndarray,
    time_to_stage: dict[str, str],
    stage_order: list[str],
) -> np.ndarray:
    s2i = {s: i for i, s in enumerate(stage_order)}
    agg = np.zeros((len(time_classes), len(stage_order)), dtype=np.float32)
    for ti, time_label in enumerate(time_classes):
        stage = time_to_stage.get(str(time_label), "")
        if stage in s2i:
            agg[ti, s2i[stage]] = 1.0
    p_stage = np.asarray(p_time, dtype=np.float32) @ agg
    row_sum = np.sum(p_stage, axis=1, keepdims=True)
    row_sum = np.where(row_sum > 0, row_sum, 1.0)
    return p_stage / row_sum


def _predict_time_and_stage(
    time_model,
    x_gene: np.ndarray,
    stage_order: list[str],
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    x_s = time_model.scaler.transform(x_gene)
    p_time = time_model.model.predict_proba(x_s).astype(np.float32, copy=False)
    time_classes = np.asarray(time_model.time_classes, dtype=object).astype(str)
    time_to_stage = {str(k): str(v) for k, v in time_model.time_to_stage.items()}
    p_stage = _time_probs_to_stage_probs(
        p_time=p_time,
        time_classes=time_classes,
        time_to_stage=time_to_stage,
        stage_order=stage_order,
    )
    pred_time = time_classes[np.argmax(p_time, axis=1)]
    pred_stage = np.asarray(stage_order, dtype=object)[np.argmax(p_stage, axis=1)]
    return p_time, p_stage, pred_time, pred_stage
...
def _build_features(
    *,
    ...
    p_time, p_stage, pred_time, pred_stage = _predict_time_and_stage(
        time_model,
    ...
    pred_time_prob = np.max(p_time, axis=1)
    pred_stage_prob = np.max(p_stage, axis=1)
    return obs_module, features, pred_time, pred_time_prob, pred_stage, pred_stage_prob, coverage
...
def predict_baseline2(
    ...
    *,
    ...
    obs, features, pred_time, pred_time_prob, pred_stage, pred_stage_prob, coverage = _build_features(
        h5ad_path=h5ad_path,
...
def _run_baseline2_for_adata(
    ...
    *,
    ...
    obs, features, pred_time, pred_time_prob, pred_stage, pred_stage_prob, coverage = _build_features_from_adata(
...
class _TimeModelLite:
    ...
    def predict_time_probs(self, x_gene: np.ndarray) -> np.ndarray:
        x_s = self.scaler.transform(x_gene)
        return self.model.predict_proba(x_s).astype(np.float32, copy=False)

    def time_probs_to_stage_probs(self, p_time: np.ndarray, stage_order: list[str]) -> np.ndarray:
        s2i = {s: i for i, s in enumerate(stage_order)}
        agg = np.zeros((len(self.time_classes), len(stage_order)), dtype=np.float32)
        for ti, time_label in enumerate(self.time_classes):
            stage = self.time_to_stage.get(str(time_label), "")
            if stage in s2i:
                agg[ti, s2i[stage]] = 1.0
        p_stage = p_time @ agg
        row_sum = np.sum(p_stage, axis=1, keepdims=True)
...
def _infer_stage_order_from_time_model(
    time_model: _TimeModelLite,
    stage_order_hint: list[str],
) -> list[str]:
    stages_seen = [str(s) for s in time_model.time_to_stage.values() if str(s)]
    order = [str(s) for s in stage_order_hint if str(s) in set(stages_seen)]
    order += sorted(set(stages_seen) - set(order))
    if not order:
        raise ValueError("No stage labels found in MLP time model.")
    return order


def _run_baseline2_time_for_adata(
    adata_query: ad.AnnData,
    *,
    model: Literal["baseline2", "baseline_tissue"],
    model_dir: str | os.PathLike | None,
    ...
    time_model = _TimeModelLite(joblib.load(run_cfg["time_model_joblib"]))
    stage_order = _infer_stage_order_from_time_model(
        time_model,
        [str(x) for x in _cfg_value(run_cfg, "stage_order", _STAGE_ORDER_DEFAULT)],
    )
    ...
    p_time = time_model.predict_time_probs(x_time.astype(np.float32, copy=False))
    p_stage = time_model.time_probs_to_stage_probs(p_time, stage_order=stage_order)
    del x_time

    pred_time = time_model.time_classes[np.argmax(p_time, axis=1)]
    pred_time_prob = np.max(p_time, axis=1)
    pred_stage = np.asarray(stage_order, dtype=object)[np.argmax(p_stage, axis=1)]
    pred_stage_prob = np.max(p_stage, axis=1)
    pred = pd.DataFrame(
        {
            "cell_id": obs_time.astype(str),
            "pred_time": pred_time,
            "pred_time_prob": pred_time_prob,
            "pred_stage": pred_stage,
            "pred_stage_prob": pred_stage_prob,
    ...
    pred.attrs["probabilities"] = p_time.astype(np.float32, copy=False)
    pred.attrs["classes"] = np.asarray(time_model.time_classes, dtype=object)
    pred.attrs["time_probabilities"] = p_time.astype(np.float32, copy=False)
    pred.attrs["time_classes"] = np.asarray(time_model.time_classes, dtype=object)
    pred.attrs["stage_probabilities"] = p_stage.astype(np.float32, copy=False)
    pred.attrs["stage_classes"] = np.asarray(stage_order, dtype=object)
    return pred, count_layer_use
...
def _time_output_prefixes(
    *,
    output: Literal["time", "stage", "both"],
    label_suffix: str | None,
    time_label: str,
    stage_label: str,
) -> dict[str, str]:
    output_use = str(output).lower().strip()
    if output_use not in {"time", "stage", "both"}:
        raise ValueError("output must be one of {'time', 'stage', 'both'}.")
    prefixes: dict[str, str] = {}
    if output_use in {"time", "both"}:
        prefixes["time"] = _base_col(f"ZMAP_{time_label}", label_suffix)
    if output_use in {"stage", "both"}:
        prefixes["stage"] = _base_col(f"ZMAP_{stage_label}", label_suffix)
    return prefixes


def _write_mlp_time_outputs(
    adata_query: ad.AnnData,
    pred: pd.DataFrame,
    *,
    output: Literal["time", "stage", "both"],
    ...
    stage_label: str,
    counts_source: str,
    apply_mask: pd.Series | np.ndarray | None = None,
    summary_key: str = "zmap_time_prediction_MLP",
) -> dict[str, str]:
    prefixes = _time_output_prefixes(
        output=output,
    ...
    if apply_mask is None:
        ...
        keep = np.asarray(apply_mask, dtype=bool)
        if keep.shape[0] != adata_query.n_obs:
            raise ValueError("apply_mask must have one value per query cell.")

    row_order = pd.Index(pred["cell_id"].astype(str))
    pos = row_order.get_indexer(adata_query.obs_names.astype(str))
    valid = pos >= 0

    for output_kind, prefix in prefixes.items():
        value_col = "pred_time" if output_kind == "time" and "pred_time" in aligned else "pred_stage"
        prob_col = (
            "pred_time_prob"
            if output_kind == "time" and "pred_time_prob" in aligned
            else "pred_stage_prob"
        )
        pred_values = aligned[value_col].to_numpy()
        prob_values = pd.to_numeric(aligned[prob_col], errors="coerce").to_numpy(dtype=float)

        adata_query.obs[f"{prefix}_unfilt"] = pred_values
        adata_query.obs[f"{prefix}_prob"] = prob_values
        if keep is None:
            adata_query.obs[prefix] = pred_values
        else:
            adata_query.obs[prefix] = pd.NA
            adata_query.obs.loc[keep, prefix] = aligned.loc[keep, value_col].to_numpy()
        adata_query.obs[f"{prefix}_unfilt"] = _string_category_for_h5ad(
            adata_query.obs[f"{prefix}_unfilt"]
        )
        adata_query.obs[prefix] = _string_category_for_h5ad(adata_query.obs[prefix])

        proba = pred.attrs.get(f"{output_kind}_probabilities")
        if proba is not None:
            ...
            adata_query.obsm[f"{prefix}_probabilities"] = out

    classes_by_output = {}
    for output_kind in prefixes:
        classes = pred.attrs.get(f"{output_kind}_classes")
        classes_by_output[output_kind] = [] if classes is None else list(map(str, classes))
    metadata = dict(pred.attrs.get("zmap_mlp_time", pred.attrs.get("zmap_baseline2", {})))
    summary = {
        "method": "MLP",
        "output": str(output),
        "output_columns": prefixes,
    ...
    adata_query.uns[summary_key] = summary
    return prefixes
...
def _write_label_transfer_outputs(
    ...
    *,
    ...
    time_output: Literal["time", "stage", "both"],
    time_label: str,
) -> None:
    ...
    if time_labels:
        ...
        if apply_filters or restrict_to_truth_labels:
            time_keep = ~adata_query.obs[col_main].isna()
        _write_mlp_time_outputs(
            adata_query,
            pred,
            output=time_output,
            label_suffix=label_suffix,
            time_label=time_label,
            stage_label=time_labels,
            counts_source=counts_source,
            apply_mask=time_keep,
        )
...
def time_prediction_MLP(
    ...
    *,
    ...
    output: Literal["time", "stage", "both"] = "both",
    time_label: str = "time",
    ...
    """
    Run the standalone MLP time/stage prediction branch.

    ``output`` controls which public columns are retained:
    ``"time"`` writes concrete time-class predictions to ``ZMAP_<time_label>...``.
    ``"stage"`` writes stage predictions to ``ZMAP_<stage_label>...``.
    ``"both"`` writes both sets of columns.
    """
    ...
    model = "baseline_tissue" if prediction_level == "tissue" else "baseline2"
    counts_source = query_raw_counts_source or counts_source
    pred, count_layer_use = _run_baseline2_time_for_adata(
        adata_query,
        model=model,
        model_dir=model_dir,
        counts_source=counts_source,
    ...
    prefixes = _write_mlp_time_outputs(
        adata_query,
        pred,
        output=output,
        label_suffix=label_suffix,
    ...
    print(
        "Finished MLP time/stage prediction: "
        + ", ".join(f"{kind}={col}" for kind, col in prefixes.items())
    )
...
def predict_labels_MLP(
    ...
    *,
    ...
    time_output: Literal["time", "stage", "both"] = "stage",
    time_label: str = "time",
    model_dir: str | os.PathLike | None = None,
    device: str = "cpu",
    projection_batch_size: int | None = None,
    ...
    ``time_output`` controls optional MLP time/stage outputs when
    ``time_labels`` is not ``None``: ``"stage"`` keeps the historical
    ``ZMAP_stage_id...`` columns, ``"time"`` writes ``ZMAP_time...`` columns,
    and ``"both"`` writes both naming schemes.
    """
    del adata_ref
    if prediction_level not in {"celltype", "tissue"}:
        raise ValueError(
...
def predict_labels_mlp(*args, **kwargs):
    """Backward-compatible alias for ``predict_labels_MLP``."""
    return predict_labels_MLP(*args, **kwargs)
