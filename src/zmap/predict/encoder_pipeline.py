...
def apply_encoder(
    ...
    *,
    ...
    with cfg_path.open() as f:
        cfg = json.load(f)
    # The encoder checkpoint stores metadata such as HVG genes and scaling
    # arrays in addition to tensor weights, so PyTorch's weights-only loader
    # cannot read legacy ZMAP checkpoints reliably.
...
def time_prediction_KNN(
    ...
    *,
    ...
    if tissue_aware_resolved and bool(predict_tissue_level):
        ...
        print(
            "[ZMAP] time_prediction_KNN: pseudo_tissue_k / pseudo_tissue_threshold are used "
    ...
    adata_query.uns["zmap_time_prediction_KNN"] = summary
    # Backward-compatible metadata key used by plotting helpers and older notebooks.
    ...
    if isinstance(teval, dict) and int(teval.get("n_valid", 0)) > 0:
        ...
        print(f"[ZMAP] time_prediction_KNN finished (time_col='{time_col}').")
    return adata_query


# Backward-compatible alias. Prefer ``time_prediction_KNN`` in new code.
time_prediction = time_prediction_KNN
